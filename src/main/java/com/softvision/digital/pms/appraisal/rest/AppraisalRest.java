...
public class AppraisalRest {
	...
	private static final String MESSAGE_STATUS_CHANGED = "Apprisal cycle has been changed to {0} status";
    ...
    public Result ready(@PathVariable(required=true) @NotNull Integer id) {
		try {
			...
			return ResultUtil.getSuccess(MessageFormat.format(MESSAGE_STATUS_CHANGED, AppraisalCycleStatus.READY));
		} catch (Exception exception) {
			return ResultUtil.getFailure(exception);
		}
    }
    ...
    public Result activate(@PathVariable(required=true) @NotNull Integer id) {
		try {
			...
			return ResultUtil.getSuccess(MessageFormat.format(MESSAGE_STATUS_CHANGED, AppraisalCycleStatus.ACTIVE));
		} catch (Exception exception) {
			return ResultUtil.getFailure(exception);
		}
    }
    ...
    public Result complete(@PathVariable(required=true) @NotNull Integer id) {
		try {
			...
			return ResultUtil.getSuccess(MessageFormat.format(MESSAGE_STATUS_CHANGED, AppraisalCycleStatus.COMPLETE));
		} catch (Exception exception) {
			return ResultUtil.getFailure(exception);
		}
    }
