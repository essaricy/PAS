...
public class PhaseAssessmentService {
	...
	public PhaseAssessmentDto getByAssignmentForSubmittedToManager(int cycleId, int employeeId, long assignId,
		...
		Optional<CycleAssignment> cycleOptional = cycleAssignmentDataRepository.findByCycleIdAndEmployeeId(cycleId, employeeId);
		CycleAssignment cycleAssignment = cycleOptional.orElseThrow(() -> new ServiceException("Invalid cycle assignment"));
		...
		Optional<PhaseAssignment> phaseOptional = phaseAssignmentDataRepository.findById(assignId);
		if (!phaseOptional.isPresent()) {
			throw new ServiceException("Phase Assignment is not found");
		}
		return getByAssignment(assignId, requestedEmployeeId, phaseOptional.get());
    }
	...
	private PhaseAssessmentDto getByAssignment(long assignmentId, int requestedEmployeeId,
        ...
        Optional<Employee> assignedToEmployee = employeeRepository.findByEmployeeId(employeeAssignment.getAssignedTo().getEmployeeId());
        if (!assignedToEmployee.isPresent()) {
        	throw new ServiceException("AssignedTo employee information is not found");
        }
        employeeAssignment.setAssignedTo(employeeMapper.getEmployeeDto(assignedToEmployee.get()));

        Optional<Employee> assignedByEmployee = employeeRepository.findByEmployeeId(employeeAssignment.getAssignedBy().getEmployeeId());
        if (!assignedByEmployee.isPresent()) {
			throw new ServiceException("Assigned by employee information is not found");
		}
        employeeAssignment.setAssignedBy(employeeMapper.getEmployeeDto(assignedByEmployee.get()));
        phaseAssessment.setEmployeeAssignment(employeeAssignment);

        int phaseId = phaseAssignment.getPhaseId();
        phaseAssessment.setPhase(appraisalMapper.getPhase(appraisalPhaseDataRepository.findById(phaseId).orElse(null)));
        ...
        if (!templateOptional.isPresent()) {
        	throw new ServiceException("Could not obtain template information");
        }
	...
	public void conclude(long assignmentId, int requestedEmployeeId, @InjectAssignment PhaseAssignment phaseAssignment)
		...
		int employeeId = phaseAssignment.getEmployeeId();
		int phaseId = phaseAssignment.getPhaseId();
        Optional<AppraisalPhase> phaseOptional = appraisalPhaseDataRepository.findNextPhase(phaseId);
        // There must be goal set for next phase. Otherwise conclude is not allowed
        AppraisalPhase nextPhase = phaseOptional.orElseThrow(() -> new ServiceException("Cannot conclude this assignment as there is no appraisal phase set up for the next period. Please infrorm your admin to set up the next phase/cycle"));

        Optional<PhaseAssignment> assignOptional = phaseAssignmentDataRepository.findByPhaseIdAndEmployeeId(nextPhase.getId(), employeeId);
        // There must be goal set for next phase. Otherwise conclude is not allowed
        assignOptional.orElseThrow(() -> new ServiceException("Assignment is not set for the next phase. You can only conclude this when assignment is set for the next phase for this employee."));
		...
		Optional<AssessHeader> optionalAssessHeader = assessmentHeaderDataRepository.findFirstByAssignIdAndStage(assignmentId,
				PhaseAssignmentStatus.CONCLUDED.getStage());
		if (!optionalAssessHeader.isPresent()) {
			throw new ServiceException("Assess header not found");
		}
		AssessHeader assessHeader = optionalAssessHeader.get();
	...
	private void updateAssessment(long assignmentId, PhaseAssignment phaseAssignment, AssessHeaderDto assessHeaderDto,
		...
		if (phaseAssignmentStatus == PhaseAssignmentStatus.SELF_APPRAISAL_PENDING
			...
			Optional<AssessHeader> assessHeaderOptional = assessmentHeaderDataRepository.findFirstByAssignIdAndStage(assignmentId,
					phaseAssignmentStatus.getStage());
			if (!assessHeaderOptional.isPresent()) {
				throw new ServiceException("Could not find assessment for the given assignment");
			}
			AssessHeader assessHeader = assessHeaderOptional.get();
	...
	public PhaseAssessmentDto getAssignmentForAudit(long assignmentId) throws ServiceException {
		Optional<PhaseAssignment> phaseAssignmentOptional = phaseAssignmentDataRepository.findById(assignmentId);
		PhaseAssignment phaseAssignment = phaseAssignmentOptional.orElseThrow(() -> new ServiceException("No such assignment exists"));
        ...
        Optional<Employee> assignedByEmployee = employeeRepository.findByEmployeeId(employeeAssignment.getAssignedBy().getEmployeeId());
        if (!assignedByEmployee.isPresent()) {
        	throw new ServiceException("AssignedBy employee information is not found");
        }
        employeeAssignment.setAssignedBy(employeeMapper.getEmployeeDto(assignedByEmployee.get()));
        phaseAssessment.setEmployeeAssignment(employeeAssignment);
