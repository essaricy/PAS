...
public class SecureAssignmentAspect {
	...
	public Object permit(ProceedingJoinPoint joinPoint, PreSecureAssignment preSecureAssignment) throws Throwable {
		...
		PhaseAssignment phaseAssignment = phaseAssignmentOptional.orElseThrow(() -> new ServiceException("No such assignment"));

		int phaseId = phaseAssignment.getPhaseId();
		Optional<AppraisalPhase> appraisalPhaseOptional = appraisalPhaseDataRepository.findById(phaseId);
		AppraisalPhase appraisalPhase = appraisalPhaseOptional.orElseThrow(() -> new ServiceException("No such appraisal phase"));
	...
	private boolean injectAnnotated(Annotation[] annotations) {
		return Arrays.stream(annotations).anyMatch(annotation ->
			annotation.annotationType() == InjectAssignment.class
		);
	}
