...
<body class="theme-red">
  ...
  <section class="content">
    <div class="container-fluid">
      <div class="block-header">
        <h2>Appraisal Status Report (Phase-wise - Assigned)
          <small>This report lists appraisal status of all the employees for whom template has been assigned.</small>
        </h2>
      </div>
  ...
  $(function () {
	...
	$('#SearchPhaseTable').DataTable({
      ...
      paging: true,
		...
		ordering: true,
		info: true,
      "ajax": "<%=request.getContextPath()%>/assignment/admin/phase/" + cycleId+"/"+phaseId,
      "sAjaxDataProp":"",
		"columns": [
  ...
  $('#SearchPhase').click(function() {
    ...
    if (cycleId ==-1) {
	  	...
	  	if(phaseId>0){
	  		table.ajax.url("<%=request.getContextPath()%>/assignment/admin/phase/" + cycleId+"/"+phaseId).load();
	  	}else{
	  		$(phaseSelectId).focus();	  	  	
	  	}
    }
